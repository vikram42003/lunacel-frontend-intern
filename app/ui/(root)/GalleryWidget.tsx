  ...
  return (
    <div className="h-full flex-col justify-between">
      <div className="flex justify-between items-center">
        <h5 className="bg-button-bg text-white py-4 px-8 rounded-2xl 3xl:text-2xl">Gallery</h5>

        <div className="flex items-center gap-8">
          <button
            className="text-white xl:text-[0.6rem] py-3.5 px-6 hover:cursor-pointer rounded-full inset-shadow-[1px_1px_5px_0px_rgba(255,255,255,0.3)] 
            shadow-[5px_5px_5px_0px_rgba(0,0,0,0.5)] 3xl:text-lg"
        ...
        <SplideTrack className="pt-4 pb-2 px-24">
          {images.map(({ id, src }) => (
            <SplideSlide key={id} className="relative rounded-2xl pt-4">
              <div className="relative w-full h-full overflow-visible rounded-2xl grayscale transition-all duration-500 ease-in-out
